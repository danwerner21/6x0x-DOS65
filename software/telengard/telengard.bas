REM Open character set file
10 OPEN #1,4,0,"D:TELEN.SET"
REM ICCOM = Read Binary
POKE 850,7
REM Buffer length = $400
POKE 856,0:POKE 857,4
REM Buffer address = $9800
POKE 852,0:POKE 853,152
REM Execute IOCB
FOUR=USR(ADR("hÅLVfl"))
 PLA REM pull argument count
 LDX #$16 REM set IOCB number
 JMP $E456 REM call CIOV
CLOSE #1

REM Point to new character set
11 POKE 756,152
...
REM Initialize arrays and variables
...
REM Generate the six stats from 3 to 18 and print the stat name and value
...
REM If player doesn’t press enter, regenerate stats
1515 GOSUB GTCHR:IF C$<>RT$ THEN 1400
REM Set hit points to constitution
1520 HP=S(THREE):
REM Set current hit points to max hit points
CH=HP:
REM Set gold and saved gold to zero
GD=ZERO:TG=GD:
REM Set Dungeon level to 1
CZ=ONE:
REM Set player level to 1
LV=CZ:
REM Clear inventory
FOR I=ONE TO TEN:I(I)=TG:SF(I)=GD:NEXT I:SF(11)=ZERO
REM Initialize box combination
1600 GOSUB 19800:
REM Set starting position in dungeon
CX=TWO:CY=70:
REM Initialize experience, spell units and current spell units
EX=ZERO:SU=ONE:CS=ONE:
REM Get players name
PRINT "}Your name noble sir? ";
1602 GOSUB 16200:IF D$="" THEN D$="DEMO":PRINT D$;
1604 NM$=D$:PRINT
REM Pad name to 7 characters
...
REM If monster is a dragon determine if he will give you something based on your charisma
3070 IF M<>C20 OR RND(ONE)*30>S(FIVE) THEN 3090
REM Randomly pick one of the first 7 items and check if it’s level is greater then the monsters
3072 I=INT(RND(ONE)*SEV+ONE):IF I(I)>=ML THEN 3090
REM Increase the level of the item
...
REM Initialize monster bonuses
3300 DB=ONE:PA=ZERO:DR=ZERO:MB=ZERO:GOTO 3330
REM RND(0-20) + Monster Level – player armor level – player shield level + monster bonus
3305 I=INT(RND(ONE)*C20)+ML-I(TWO)-I(THREE)+MB
REM If SPECTER, VAMPIRE, or DEMON and protection from evil spell you get a bonus of -6 to hit
3306 IF M>16 AND M<C20 AND SF(FOUR)>ZERO THEN I=I-FOUR-TWO
REM Check for hit
3310 IF I<TEN THEN PRINT "It missed...";:GOSUB WTCLR:GOTO 3100
REM Damage = (RND(0-8) + RND() * Monster level * 2 + 0) * Monster Bonus
3315 I=INT((RND(ONE)*EIG+RND(ONE)*ML*TWO+ONE)*DB):PRINT "It does ";I;" points damage";
REM Reduce player hit points and check for death
3320 CH=CH-I:GOSUB RHITS:IF CH<ONE THEN 9000
3321 GOSUB PAUSE:
REM Check for special effects
...
REM Skip to items 50% of the time
4100 IF RND(ONE)>ONE/TWO THEN 4200
REM Handle treasure chest
...
REM Calculate amount of gold in chest
4140 GOSUB WTCLR:I=INT(RND(ONE)*1000*L TWO+ONE)
REM 10% of the time there is nothing inside
...
REM 50% of the time evaded monsters will get closer
...
REM 8 = Throne
REM 9 = Box
REM Get items one level up
6000 X=CX:Y=CY:Z=CZ-ONE:GOSUB 10010:J=USR(FNS,HI):
REM Get item on current level
I=USR(FNS,LC)
REM If on level one, nothing one level up */
6005 IF CZ=ONE THEN J=ZERO
REM Be sure special item is less then 9 */
6010 Q=FOUR+FIVE:IF I>Q THEN I=I-Q:GOTO 6010
6012 IF J>Q THEN J=J-Q:GOTO 6012
REM If on level 50, no down stairs */
6013 IF CZ=FIVE*TEN AND I=FOUR THEN I=ZERO
REM No item on this level, no stair on previous, then no special */
6014 IF I=ZERO AND J<>FOUR THEN GOTO ARRW
REM Stair on previous level, do stairs */
6015 IF J=FOUR THEN GOTO 6500
REM Do special objects*/
...
";IN2$;" ";IN3$
REM Add carried gold to expereince and total gold, then clear to 0
...
REM Clear all spell effects
6116 FOR I=ZERO TO 11:SF(I)=ZERO:NEXT I
REM Restore spell points
6119 CS=SU:PRINT "You spend the night":GOSUB PAUSE
REM Prompt for save or re-enter dungeon
...
REM Fell in pit. Handle damage and move down one level
6330 PRINT "You fall in!!":L=THREE:GOSUB NEWP0:GOSUB PAUSE
6335 CZ=CZ+ONE:GOTO NEWP
REM**************************************************************
REM Teleporter
REM**************************************************************
REM Display message and clear monster stack
...
6532 IF C$="&" THEN C$="S"
REM Check if you can go in the select direction
...
REM Don’t workship. 30% of the time create an undead monster
...
REM Clear screen and continue game
6710 GOSUB CLWND:GOTO ARRW
REM 3/5 of the time something skip to special types of fountain
6715 GOSUB CLWND:IF RND(ONE)>THREE/FIVE THEN 6750
REM Skip healing effect
6717 IF RND(ONE)>ONE-C*THREE/C20 THEN 6730
REM Healing fountain. Add (3 * dungeon level) + 1 to hit points
...
REM Skip poison fountain
6730 IF RND(ONE)>THREE*C/C20 THEN 6745
REM Poison Fountain. Subtract (3 * dungeon level) + 1 from hit points. Check if dead
...
REM Handle lose item fountain. Remove all items
...
REM 80% of the time you can pick the level to go to
6810 IF RND(ONE)>ONE/FIVE THEN 6815
REM Goto random level
6812 CZ=INT(RND(ONE)*FIVE*TEN+ONE):GOTO 6820
REM Get level number to goto
...
REM 30% of the time a monster appears
6925 PRINT "Pry";:GOSUB WTCLR:IF RND(ONE)>SEV/TEN THEN 7030
REM 60% of the time nothing happens
6930 IF RND(ONE)>TWO/FIVE THEN 7025
REM Got the jewel, increase gold
...
REM 40% of the time nothing happens
6960 IF RND(ONE)>THREE/FIVE THEN 7025
REM 60% of the time your are teleported
6965 IF RND(ONE)>TWO/FIVE THEN 6403
REM 50% of the time you loose ½ experiece
...
REM If your level is less then or equal to the dungeon level, then you get an experience bonus
...
REM 30% of the time create a monster
6992 IF RND(ONE)>SEV/TEN THEN 7030
REM Determine success based on player’s intelligence
...
REM Get input from player
8005 PRINT "->";:GOSUB GTCHR:IF C$="&" THEN C$="S"
REM See if it’s a valid command
...
REM If you stay evaded monsters get closer
...
REM IF Q & 0x03 <> 0 then goto 10030
10020 IF USR(LG,ONE,Q,THREE)<>ZERO THEN 10030
REM Q = Q << 2
10022 Q=Q/FOUR:
REM Q = Q & 0x0F
Q=USR(LG,ONE,Q,15):
REM If Q > 9 then Q = Q - 9
IF Q>NIN THEN Q=Q-NIN
REM HI = HI + (Q >>8)
10024 HI=HI+Q*TF
REM If at left or right edge of maze turn on left wall
10030 IF X=ONE OR X=201 THEN HI=USR(LG,TWO,HI,12)
REM if at top of bottom of maze turn on top wall
...
REM Display any spells in effect
...
REM Plot section 0,0 upper wall
...
REM Plot section 0,0 left wall
...
REMPlot section 0,1 upper and left wall and section 0,2 left wall
...
REMPlot section 0,2 upper wall
...
REM Plot section 0,3 left wall
...
REM Plot section 1,2 upper wall, section 1,3 left wall, section 2,2 upper wall
...
REM Plot section 3,2 upper wall
...
REM Plot section 2,3 left wall
...
REM Plot section 2,2 left wall, 3,1 upper wall, and 2,1 left wall
...
REM Plot section 3,0 upperwall
...
REM Plot section 2,0 left wall
...
REM Plot section 2,0 upper wall, section 1,0 left wall, and 1,0 upper wall
...
15499 RETURN
REM*******************************************************
REM GTCHR: Read a character from the keyboard
REM*******************************************************
REM Loop for 50 counts or until key is pressed
16000 C$=STR$(Q): FOR Q=ONE TO 50: IF PEEK(764)=255 THEN 16010
REM Get the key that was pressed and put it in C$
16005 GET #ONE,Q:C$=CHR$(Q):RETURN
REM Loop back and try again
16010 NEXT Q:IF NM$="DEMO " THEN GOTO GTCHR
REM Return & if no key was pressed
16015 C$="&":RETURN
REM***************************************************
REM Get multi character input from player
REM***************************************************
16200 D$=""
...
REM Gain/lose experience points
20100 I=INT(RND(ONE)*500*CZ+ONE):PRINT "You just ";
REM 50% of the time you should lose experience points. This line appears to have a bug, I=-I1 should be
...
REM Protection from evil spell
...
REM Invisibility spell
24400 T=SIX:U=C20+ONE:SETCOLOR TWO,ZERO,TWO:GOTO 22402
REM Hold monster spell
24500 GOSUB WTCLR:IF D=ZERO THEN 21800
REM Check monster level against Intelligence to see if spell succeeds
...
REM Calculate chance of kill succeeding
24520 IF RND(ONE)>ML*6.0E-3 THEN GOTO MOND
REM Kill did not succeed
24525 PRINT "It broke free!!":GOSUB PAUSE:GOTO 3300
REM Phantasmal Force spell
...
REM Astral walk spell
26200 T=EIG:U=T*TWO:GOTO 22402
REM Power word kill spell
...
REM Get spell name, spell number in S
...
REM Generate random Inn description
...
REM Return item name for item number in MI
...
REM Pull a monster off the monster stack
...
LG:
0600: 68 PLA REM Pull number of parameters
0601: d0 2a BNE $062d REM Branch if it’s not zero
0603: 20 23 06 JSR $0623 REM Pull the rest of the params off the stack
0606: a9 00 LDA #$00 REM Return 0x0001
0608: 85 d4 STA $d4
060a: a9 01 LDA #$01
060c: 85 d5 STA $d5
060e: 60 RTS
060f: 20 23 06 JSR $0623 REM Pull the rest of the params off the stack
0612: a9 02 LDA #$02 REM Return 0x0202
0614: 85 d4 STA $d4
0616: 85 d5 STA $d5
0618: 60 RTS

0619: 20 23 06 JSR $0623 REM Pull the rest of the params off the stack
061c: a9 01 LDA #$01 REM Return 0x0101
061e: 85 d4 STA $d4
0620: 85 d5 STA $d5
0622: 60 RTS
REM Pull the rest of the parameters off the stack
0623: 8a TXA
0624: f0 06 BEQ $062c
0626: 68 PLA
0627: 68 PLA
...
062d: aa TAX REM X = number of parameters
062e: 68 PLA REM Pull high byte
062f: 68 PLA REM Pull low byte
0630: ca DEX REM next parameters
0631: f0 e6 BEQ $0619 REM Finished all parameters
0633: a8 TAY REM Y = low byte
0634: d0 0f BNE $0645 REM Branch if low byte <> 0
0636: e0 01 CPX #$01 REM Branch if not on parameter 1
0638: d0 d5 BNE $060f REM
063a: 68 PLA REM Get parameters
063b: 68 PLA REM
063c: 49 ff EOR #$ff REM Invert it
063e: 85 d4 STA $d4 REM return it
0640: a9 00 LDA #$00
0642: 85 d5 STA $d5
0644: 60 RTS
REM AND the 2 parameters together and return
0645: c0 01 CPY #$01 REM First parameter = 1
0647: d0 1e BNE $0667 REM branch if not
0649: e0 02 CPX #$02 REM branch if less then 2 parameters left
064b: 30 c2 BMI $060f REM
064d: a9 ff LDA #$ff
064f: 8d ff 06 STA $06ff
0652: 68 PLA REM High byte of parameters
0653: 68 PLA REM Low byte of parameters
0654: 2d ff 06 AND $06ff REM AND with other parameters
0657: 8d ff 06 STA $06ff
065a: ca DEX REM next parameter
065b: d0 f5 BNE $0652 REM
065d: ad ff 06 LDA $06ff REM Return value
0660: 85 d4 STA $d4 REM
0662: a9 00 LDA #$00 REM
0664: 85 d5 STA $d5 REM
0666: 60 RTS REM
REM AND the Inverse of the 2 parameters together and return
0667: c0 02 CPY #$02 REM First parameter = 2
0669: d0 22 BNE $068d REM branch if not
066b: e0 02 CPX #$02 REM branch is less then 2 parameters left
066d: 30 a0 BMI $060f REM
066f: a9 ff LDA #$ff
0671: 8d ff 06 STA $06ff
0674: 68 PLA REM High byte param 2
0675: 68 PLA REM Low byte param 2
0676: 49 ff EOR #$ff REM Invert it
0678: 2d ff 06 AND $06ff REM AND with other parameter
067b: 8d ff 06 STA $06ff
067e: ca DEX REM next parameter
067f: d0 f3 BNE $0674 REM
0681: ad ff 06 LDA $06ff REM Invert the result
0684: 49 ff EOR #$ff REM
0686: 85 d4 STA $d4 REM Return value
0688: a9 00 LDA #$00 REM
068a: 85 d5 STA $d5 REM
068c: 60 RTS REM
...
FNUP:
06a2: 68 PLA REM Number of parameters
06a3: aa TAX
06a4: e0 01 CPX #$01 REM Branch if 1 parameter
06a6: f0 03 BEQ $06ab REM
06a8: 4c 0f 06 JMP $060f
06ab: 68 PLA REM Read parameter
06ac: 68 PLA REM
06ad: 29 03 AND #$03 REM And low byte with #$03
06af: 85 d4 STA $d4 REM Store in return register
06b1: a9 00 LDA #$00
06b3: 85 d5 STA $d5
06b5: 60 RTS
Purpose: Returns bits 2-3 of value in 1st parameter
FNLF:
06b6: 68 PLA REM Number of parameters
06b7: aa TAX
06b8: e0 01 CPX #$01 REM Branch if 1 parameter
06ba: f0 03 BEQ $06bf
06bc: 4c 0f 06 JMP $060f
06bf: 68 PLA REM Read parameters
06c0: 68 PLA REM
06c1: 4a LSR REM Shift right twice
06c2: 4a LSR
06c3: 29 03 AND #$03
06c5: 85 d4 STA $d4
06c7: a9 00 LDA #$00
...
1740:
06cc: 68 PLA REM Pull parameter count (always 0)
06cd: a9 3f LDA #$3f
06cf: 85 d4 STA $d4
06d1: a9 9c LDA #$9c
06d3: 85 d5 STA $d5
